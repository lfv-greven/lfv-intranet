    ...
    <div class="max-w-4xl mx-auto space-y-2 mt-8">
        @foreach($event->slots as $slot)
            <label for="enrollment_{{ $slot->id }}"
                   class="flex items-center justify-between rounded border p-4 bg-gray-50  @if($event->enrollment?->event_slot_id == $slot->id) ring ring-green-500 border-green-500 @endif @if($slot->free_seats <= 0) opacity-50 @else hover:ring ring-gray-100 @endif">
                <span class="flex items-center space-x-4">
                    <input type="radio"
                           ...
                           @if($slot->free_seats <= 0) disabled @endif
